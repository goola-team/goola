...
import (
	...
	"github.com/goola-team/goola/accounts/usbwallet/internal/trezor"
...
// or whether the Goola app was not started on it.
...
// Goola address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.GoolaAddress)
	if _, err := w.trezorExchange(&trezor.GoolaGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.GoolaTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.GoolaTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
