...
// faucet is a Goola faucet backed by a light client.
package main
...
var (
	...
	ethPortFlag = flag.Int("goolaport", 31318, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Goola protocol")
	statsFlag   = flag.String("goolastats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Goolas to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Goola and the faucet")
)

var (
	goolaFund = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Goolas", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Goola address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Goola light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Goola protocol stack
	client *goolaclient.Client // Client connection to the Goola chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Goola light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := goolabackend.DefaultConfig
	...
	// Assemble the goolastats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightGoola
			ctx.Service(&serv)
			return goolastats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := goolaclient.NewClient(api)
...
// close terminates the Goola connection and tears down the faucet.
...
// apiHandler handles requests for Goola grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, goolaFund),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Goola address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/goola-team/goola/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), goolaFund)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, goolaFund)
...
// the username, avatar URL and Goola address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Goola addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Goola address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Goola address and profile URL.
...
// returning the username, avatar URL and Goola address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Goola address,
