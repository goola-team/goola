...
func newTester(t *testing.T, confOverride func(*goolabackend.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Goola service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	ethConf := &goolabackend.Config{
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return goolabackend.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Goola protocol: %v", err)
	}
	...
	var goola *goolabackend.FullGoola
	stack.Service(&goola)
