...
// PublicEthereumAPI provides an API to access Goola related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEthereumAPI struct {
	b Backend
}

// NewPublicEthereumAPI creates a new Goola protocol API.
...
// ProtocolVersion returns the current Goola protocol version this node supports
...
// Sign calculates an Goola ECDSA signature for:
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Goola signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Goola blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Goola blockchain API.
...
// PublicDebugAPI is the collection of Goola APIs exposed over the public
// debugging endpoint.
...
// Version returns the current Goola protocol version.
