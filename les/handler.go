...
const (
	...
	ethVersion = 63 // equivalent goola version for the downloader
...
// NewProtocolManager returns a new Goola sub protocol manager. The Goola sub protocol manages peers capable
// with the Goola network.
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb gooladb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Goola protocol")
	...
	log.Info("Light Goola protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Goola peer connected", "name", p.Name())
	...
	if err := p.Handshake( hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Goola handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Goola peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Goola message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Goola message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(gooladb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				if root, prefix = pm.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(gooladb.NewTable(pm.chainDb, prefix)))
				}
			}
...
// NodeInfo represents a short summary of the Goola sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Goola network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
