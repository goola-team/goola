...
import (
	...
	"github.com/goola-team/goola/les/flowcontrol"
...
func NewLesServer(backend *goolabackend.FullGoola, config *goolabackend.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(backend.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, backend.EventMux(), backend.Engine(), newPeerSet(), backend.BlockChain(), backend.TxPool(), backend.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(backend.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(backend.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(backend.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(backend.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(backend.ChainDb())
	return srv, nil
}
...
func newCostStats(db gooladb.Database) *requestCostStats {
