...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, backend Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:      config,
		engine:      engine,
		backend:     backend,
	...
	worker.txSub = backend.TxPool().SubscribeTxPreEvent(worker.txCh)
