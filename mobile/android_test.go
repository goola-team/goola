...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", goola.LightScryptN, goola.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			GoolaClient ec = node.getEthereumClient();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "goola-android-")
	if err != nil {
	...
	// Generate the mobile bindings for goola and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/goola-team/goola/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "goola.aar"), "goola.aar", os.ModePerm)
