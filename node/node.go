...
func (n *Node) OpenDatabase(name string, cache, handles int) (gooladb.Database, error) {
	if n.config.DataDir == "" {
		return gooladb.NewMemDatabase()
	}
	return gooladb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
