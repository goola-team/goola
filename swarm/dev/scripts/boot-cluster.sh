...
#
# The cluster contains a bootnode, a goola node and multiple swarm nodes, with
...
# static goola configuration
GOOLA_IP="192.168.33.3"
GOOLA_RPC_PORT="9687"
GOOLA_RPC_URL="http://${GOOLA_IP}:${GOOLA_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the goola and
# swarm nodes
start_bootnode() {
...
# start_goola_node starts a goola node with --datadir pointing at <base-dir>/goola
# and a single, unlocked account with password "goola"
start_goola_node() {
  local dir="${base_dir}/goola"
  mkdir -p "${dir}"

  local password="goola"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating goola account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get goola account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GOOLA_IP}"
    --rpcport   "${GOOLA_RPC_PORT}"
    --verbosity "6"
  )

  start_node "goola" "${GOOLA_IP}" "$(which goola)" ${args[@]}
}
...
create_account() {
  ...
  goola --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
